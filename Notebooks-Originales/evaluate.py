...
def get_precision(model, X, Y, original_test_samples):  #Precisión = VP / (VP + FP),  la precisión mide cuántas de las predicciones positivas realmente lo son; son correctas
    ...
    output = model(X[0], X[1], X[2]).view(-1)
    y_pred = torch.sigmoid(output) >= 0.5  # Predicciones binarias
    Y = Y >= 0.5  # Etiquetas binarias
    ...
    for i in range(len(y_pred)):
        if y_pred[i] == 1 and Y[i] == 1:  # Verdaderos positivos
           v_tp.append(original_test_samples[i])
        if y_pred[i] == 1 and Y[i] == 0:  # Falsos positivos
           v_fp.append(original_test_samples[i])
...
def get_recall(model, X, Y, original_test_samples): #Recall = VP / VP + FN, mide la prop de positivos reales que el modelo identifica corrctamente
    ...
    for i in range(len(y_pred)):     
        if y_pred[i] == 1 and Y[i] == 1:  # Verdaderos positivos
            v_tp.append(original_test_samples[i])
        if y_pred[i] == 0 and Y[i] == 1:  # Falsos negativos
            v_fn.append(original_test_samples[i])
